...
ARGS=$(getopt -o i:p:o:f::r:x: --long input_sysname:,input_path:,output_sysname:,ff_path:,rep:,extra_itp:,dry-run:,notrj -- "$@")
...
# Check that forcefield_folder exists
if [ ! -d "$ff_path" ]; then
  echo "Error: forcefield_folder '$ff_path' does not exist"
  exit 1
fi
