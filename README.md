# hoarder.sh: a Helpful OMara-to-ACSC-Repository Dataset Exporter & Reshaper

`hoarder.sh` is a script to automatically reshape a molecular dynamics simulation from a typical O'Mara group directory structure into a directory structure suitable for publishing on the [Australasian Computational Simulation Commons](https://molecular-dynamics.atb.uq.edu.au).  After a dataset is processed with Hoarder it should be validated and archived with [CSC tools](https://atb-uq.github.io/csc-tools/), and submitted to the repository.
...
This should be a short, human-readable title, that informs the user what the simulation was about.  With the exception of the replicate number, it should be consistent between every simulation replicate of your system.  It should end with the replicate number in the format `{replicate X of Y}`.

### Metadata notes

This should be a short human readable description the system.  If the simulation was published, this should include the DOI of the relevant paper.  This should be only a brief summary of what the simulation system is; don't include excessive detail that is already included in the paper.  It should also end with the replicate number in the format `{replicate X of Y}`.
...
* You should also include other commonly used references to your protein, such as a uniprot id.  For human GlyT2, you would also include `protein-SLC6A5` and `uniprot-Q9Y345`
